...
func TestValidateLongUrl_ValidHttp(t *testing.T) {
	assert.NoError(t, businesslogic.ValidateLongUrl("http://example.com/path?q=1", 2048))
}

func TestValidateLongUrl_ValidHttps(t *testing.T) {
	assert.NoError(t, businesslogic.ValidateLongUrl("https://www.example.com", 2048))
}

func TestValidateLongUrl_TooLong(t *testing.T) {
	raw := "https://example.com/" + strings.Repeat("a", 2048)
	err := businesslogic.ValidateLongUrl(raw, 2048)
	require.Error(t, err)
...
func TestValidateLongUrl_NoHost(t *testing.T) {
	err := businesslogic.ValidateLongUrl("https://", 2048)
	require.Error(t, err)
	assert.ErrorIs(t, err, businesslogic.ErrValidation)
}
...
func TestValidateLongUrl_LoopbackIP(t *testing.T) {
	err := businesslogic.ValidateLongUrl("http://127.0.0.1/evil", 2048)
	require.Error(t, err)
	assert.ErrorIs(t, err, businesslogic.ErrValidation)
}

func TestValidateLongUrl_IPv6Loopback(t *testing.T) {
	err := businesslogic.ValidateLongUrl("http://[::1]/evil", 2048)
	require.Error(t, err)
	assert.ErrorIs(t, err, businesslogic.ErrValidation)
}
...
func TestValidateLongUrl_PublicIP_Valid(t *testing.T) {
	...
	assert.NoError(t, businesslogic.ValidateLongUrl("https://8.8.8.8", 2048))
}
